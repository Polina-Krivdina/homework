...
/**
 * Комментарии по принципам DRY, KISS и YAGNI:
 *
 * 1. **DRY (Don't Repeat Yourself)**:
 *    - **Проблема:** Множественные методы для одного и того же действия с небольшими вариациями:
 *        ```java
 *        static void addArticle(D database)
 *        static void addArticle(String title, String content, D database)
 *        ```
 *      **Исправление:** Объединить эти методы в один, чтобы избежать дублирования.
 *
 *    - **Проблема:** Множественные методы для просмотра и листинга статей/пользователей с похожей логикой:
 *        ```java
 *        static void viewArticles(D database)
 *        static void viewAllArticles(D database)
 *        static void listAllUsers(D database)
 *        ```
 *      **Исправление:** Объединить эти методы для предотвращения дублирования.
 *
 * 2. **KISS (Keep It Simple, Stupid)**:
 *    - **Проблема:** Сложная структура главного меню и дублирующиеся опции:
 *        ```java
 *        System.out.println("9. Добавить статью");
 *        System.out.println("10. Посмотреть все статьи");
 *        ```
 *      **Исправление:** Упростить структуру меню и убедиться, что опции лаконичны и не дублируются.
 *
 *    - **Проблема:** Сложные кейсы switch для управления пользователями:
 *        ```java
 *        static void manageUsers(D database)
 *        ```
 *      **Исправление:** Разбить на более простые методы и сделать switch кейсы более читабельными.
 *
 * 3. **YAGNI (You Aren't Gonna Need It)**:
 *    - **Проблема:** Слишком сложный метод для изменения ролей пользователей, не используемый напрямую в процессе:
 *        ```java
 *        static void changeUserRole(long userId, Role newRole, D database)
 *        ```
 *      **Исправление:** Включать только ту функциональность, которая в данный момент требуется. Удалить или закомментировать неиспользуемые методы до тех пор, пока они не понадобятся.
 *
 * Внедрение этих исправлений сделает код чище, проще в сопровождении и понятнее.
 */
